...
public class YarnCommandManager : DialogueViewBase
{
	...
	public Image fadeBG, nameplateBG;
	...
	[HideInInspector] public Dictionary<string, Yarn.Unity.Example.VNActor> actors = new Dictionary<string, Yarn.Unity.Example.VNActor>(); // tracks names to sprites

	static Vector2 screenSize = new Vector2( 1280f, 720f); // needed for position calcuations, e.g. what does "left" mean?

	//keep track of current evidence responses
	private List<EvidenceResponse> evidenceResponses = new List<EvidenceResponse>();
	...
	void Awake () {
		...
		GameManager.Instance.SetYarnCommandManager(this);
		...
		// Unused sprite commands from starter code
	...
	{
		evidenceResponses.Clear();
	}

	public void SetEvidenceResponse(string evidence, string node)
	{
...
//
	// 	// save actor data
	// 	actors.Add( actorName, new Yarn.Unity.Example.VNActor( newActor, actorColor) );
	// }
//
...
//
	...
	// 	
	// 	var wildcard = new Yarn.Unity.Example.Wildcard(spriteName);
//
